...
func _on_node_2d_child_entered_tree(node: Node) -> void:
	...
	var tile_res = "res://Map_data/armies/" + name_of_army_file + ".json"
	print(tile_res)
	print(tile_located_on)
	var tile_file = FileAccess.open(tile_res, FileAccess.READ)
	print(tile_file)
	var army_text = tile_file.get_as_text()
	tile_file.close
	var army_parse = JSON.parse_string(army_text)
	infantry_num += 1
	army_parse["tile_located_on"] = tile_located_on
	
	tile_file = FileAccess.open(tile_res, FileAccess.WRITE)
	tile_file.store_string(JSON.stringify(army_parse, "\t"))
	tile_file.close()
